...
import com.diganta.nextgen.NextGenApp.Companion.showQuickstepWarningIfNecessary
...
class NextGenLauncher : QuickstepLauncher() {
    ...
    private val statusBarClockListener = object : StateManager.StateListener<LauncherState> {
        override fun onStateTransitionStart(toState: LauncherState) {
            when (toState) {
                ...
                is AllAppsState,
                -> {
                    NextGenApp.instance.restoreClockInStatusBar()
                }
                else -> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        layoutInflater.factory2 = app.lawnchair.LawnchairLayoutFactory(this)
        ...
        if (prefs.autoLaunchRoot.get()) {
            lifecycleScope.launch {
                try {
                    RootHelperManager.INSTANCE.get(this@NextGenLauncher)
    ...
    override fun getSupportedShortcuts(): Stream<SystemShortcut.Factory<*>> = Stream.concat(
        ...
        Stream.concat(
            ...
            if (NextGenApp.isRecentsEnabled) Stream.of(LawnchairShortcut.PAUSE_APPS) else Stream.empty(),
        ),
    )
    ...
    override fun handleGestureContract(intent: Intent?) {
        if (!NextGenApp.isRecentsEnabled) {
